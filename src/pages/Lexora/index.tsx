  ...
  return <div className="overflow-hidden bg-ts-canvas text-ts-ink">
    <section className="relative">
      <div className="relative mx-auto grid max-w-7xl gap-12 px-6 py-20 md:grid-cols-2 md:px-12 md:py-28 lg:gap-20">
        ...
        <div className="relative mx-auto w-full max-w-[640px] self-center"><img src="/assets/lexora/ai-result.jpg" alt="Lexora 浏览器扩展解读界面" className="relative w-full rounded-2xl border border-white/60 dark:border-white/10 bg-white/60 dark:bg-ts-ink/5 backdrop-blur-2xl shadow-2xl shadow-ts-ink/10 transition-all duration-300 hover:-translate-y-1 hover:shadow-ts-ink/20" /></div>
      </div>
    </section>
    ...
    <section className="mx-auto grid max-w-7xl gap-10 px-6 py-20 md:grid-cols-[0.75fr_1.25fr] md:px-12 md:py-28">
      ...
      <div className="grid gap-4 sm:grid-cols-3">{copy.features.map(([title, text], index) => <article key={title} className="rounded-2xl border border-white/60 dark:border-white/10 bg-white/60 dark:bg-ts-ink/5 backdrop-blur-2xl shadow-xl shadow-ts-ink/5 p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-ts-ink/10"><div className="mb-8 text-ts-ink">{index === 0 ? <MousePointer2 size={22} /> : index === 1 ? <Layers3 size={22} /> : <MessageCircle size={22} />}</div><h3 className="font-semibold">{title}</h3><p className="mt-3 text-sm leading-6 text-ts-muted">{text}</p></article>)}</div>
    </section>

    <section id="how-it-works"><div className="mx-auto max-w-7xl px-6 py-20 md:px-12 md:py-28"><p className="text-xs font-semibold tracking-[0.18em] text-ts-ink">{copy.flow}</p><div className="mt-10 grid gap-5 md:grid-cols-3">{copy.steps.map(([number, title, text]) => <article key={number} className="rounded-2xl border border-white/60 dark:border-white/10 bg-white/60 dark:bg-ts-ink/5 backdrop-blur-2xl shadow-xl shadow-ts-ink/5 p-7 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-ts-ink/10"><span className="text-sm font-semibold text-ts-ink">{number}</span><h3 className="mt-10 text-xl font-semibold">{title}</h3><p className="mt-3 leading-7 text-ts-muted">{text}</p></article>)}</div></div></section>

    <section className="mx-auto grid max-w-7xl gap-10 px-6 py-20 md:grid-cols-2 md:px-12 md:py-28"><div className="overflow-hidden rounded-2xl border border-white/60 dark:border-white/10 bg-white/60 dark:bg-ts-ink/5 backdrop-blur-2xl shadow-xl shadow-ts-ink/5 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-ts-ink/10"><img src="/assets/lexora/selection.jpg" alt="Lexora 划词后显示 L 解读入口" className="h-full w-full object-cover" /></div><div className="flex flex-col justify-center"><BookOpen className="text-ts-ink" size={25} /><h2 className="mt-5 text-3xl font-semibold tracking-[-0.035em] md:text-4xl">{copy.madeFor}</h2><p className="mt-5 max-w-lg leading-8 text-ts-muted">{copy.madeForText}</p><div className="mt-8 flex items-start gap-3 rounded-xl border border-white/60 dark:border-white/10 bg-white/60 dark:bg-ts-ink/5 backdrop-blur-2xl shadow-xl shadow-ts-ink/5 p-4 text-sm leading-6 text-ts-muted transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-ts-ink/10"><Check className="mt-0.5 shrink-0 text-ts-ink" size={17} />{copy.sourceText}</div><h3 className="mt-7 font-semibold">{copy.sourceTitle}</h3></div></section>
